...
def executar_programa(file_path, pasta_saida, mes_referencia, ano_4):
    ...
    if not file_path.lower().endswith(".xlsx"):
        raise ValueError("Somente arquivos .xlsx são permitidos. Converta o arquivo .xls para .xlsx antes de importar.")
    ...
    situacoes = df["SITUACAO"].unique()

    for situacao in situacoes:
        df_situacao = df[df["SITUACAO"] == situacao].copy()
        linhas = []
        ...
        for _, row in df_situacao.iterrows():
            ...
            if data_obito_dt.year == int(ano_4) and data_obito_dt.month == int(mes_referencia):
                ...
                # --------------------------
                # GERA DATA LIMITE + C86/B86
                # --------------------------

                if situacao == "4 - MILITAR VETERANO":
                    data_formatada = data_obito
                elif situacao == "5 - PENSIONISTA":
                ...
                # --------------------------
                # QUALQUER OUTRA SITUAÇÃO → cálculo_3
                # --------------------------
                linha_calculo_3 = (
        ...
        if linhas:
            ...
            output_file = os.path.join(
                pasta_saida,
                f"arquivo_para_importacao_{situacao.lower().replace(' ', '_')}.txt"
            )
...
tk.Label(janela, text="Arquivo Excel (.xls ou .xlsx):").pack()
